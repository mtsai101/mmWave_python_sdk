...
with open("detected.csv",'a') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['frame','x', 'y', 'z','numObj', 'rangeIdx', 'range','dopplerIdx', 'doppler', 'peakVal'])
...
def update():
    ...
    if dataOk and len(detObj["x"]) > 0:
        ...
        moving_obj = rm_background(newdata)
        # pos[:][:newdata.shape[0]] = moving_obj
        # s.setData(pos = pos)
...
def rm_background(rawdata):
    global prev_frame
    cur_point_frame = rawdata.round(decimals=2)
